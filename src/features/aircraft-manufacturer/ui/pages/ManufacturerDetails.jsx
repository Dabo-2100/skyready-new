...
export default function ManufacturerDetails() {
  ...
  const { data: manufacturers, isLoading: isLoadingManufacturers } = useQuery({ queryKey: ["manufacturers"], queryFn: () => AircraftRepo.index_manufacturers(), ...noRefreshState });
  const { data: manufacturer, isLoading: isLoadingManufacturer } = useQuery({ queryKey: ["manufacturer", manufacturer_id], queryFn: () => AircraftRepo.show_manufacturer(manufacturer_id), ...noRefreshState });
  ...
  const validationSchema = Yup.object({
    name: Yup.string()
      .test("unique-manufacturer-name", "Manufacturer already exists", function (value) {
        ...
        return !manufacturers?.some((m) => m.name.toLowerCase().trim() === normalized);
      })
