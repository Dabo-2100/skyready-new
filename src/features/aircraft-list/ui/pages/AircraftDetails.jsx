...
import { AircraftRepo } from "../../../fleet/data/AircraftRepo";
...
export default function AircraftDetails() {
  ...
  const { data: manufacturers } = useQuery({ queryKey: ["manufacturers"], queryFn: AircraftRepo.index_manufacturers, ...noRefreshState });
  ...
  const validationSchema = Yup.object({
    ...
    customerNo: Yup.string().required("Registration No is required"),
  ...
  const initialValues = {
    ...
    customerNo: aircraftInfo?.customerNo || "",
    vendorId: vendorId ?? "",
    typeId: typeId ?? "",
  ...
  useEffect(() => {
    if (aircraftInfo?.aircraft_model?.aircraft_type) {
      ...
      const vendor = type.manufacturer;
  ...
  return (
    <div className="flex flex-col w-full h-full overflow-auto">
      ...
      <div className="w-full p-3 flex justify-between items-center border-b border-b-gray-200">
        ...
        {UserAuthorties().canUpdate("fleet") && (
      ...
      <div className="w-full p-3">
        <h1 className="w-full bg-gray-200 text-lg font-semibold p-4">Basic Info</h1>
          ...
          {({ values }) => (
            <Form className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="flex flex-col gap-2 border border-gray-200 p-3">
                ...
                {isEdit ? <Field name="customerNo" className="my-input" /> : <p>{values.customerNo}</p>}
                <ErrorMessage name="customerNo" component="div" className="text-red-500" />
              </div>
      ...
      <div className="p-3 w-full">
        <h1 className="w-full bg-gray-200 text-lg font-semibold p-4">Powerplant & Drive Systems</h1>
      </div>

      <div className="w-full p-3 flex justify-end">
        {UserAuthorties().canDelete("fleet") && (
